...
var Storage = Class.extend({
	...
	*/
	...
	validateData: function (req) {
		...
		if (!rules) {
			return [{message: "Model does not exist `"+req.table+"`."}];
		}
		...
		for (var key in data) {
			...
			if (error.length) {
				for (var i = 0; i < error.length; ++i) {
					errors.push({message: englishKey(key) + ": " + error[i]})
				}
			}
			...
			if (this.inheritRole(permission, access) === false) {
				console.log("FIELD PERMISSIONS", key, permission, access, this.inheritRole(permission, access));
				delete data[key];
			}
		}
	...
	post: function (req, next) {
		...
		if (req.permission && req.permission != "anyone" && req.permission != "stranger" && !req.session.user) {
			return next([{message: "Must be logged in"}]);
		}
