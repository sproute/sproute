	...
	setupRoutes: function (app) {
		...
		app.server.get("/admin/views", function (req, res) {
			var f = ff(this, function () {
				app.fs.readdirRecursive(path.join(app.dir, app.config.views), f.slot())
			}, function (dirs) {
		...
		app.server.get("/admin/views/:name", function (req, res) {
			ff(function () {
				...
				if (err) {
					return res.json(500, [{message: "No view found."}]);
				}
		...
		app.server.post("/admin/views", function (req, res) {
			...
			if (!Validation.testFieldName(basename)) {
				return res.json(500, [{message: "Invalid template name"}]);
			}
		...
		app.server.post("/admin/config", function (req, res) {
			...
			if (c.url) {
				...
				// if not subdomain
				if (app.config.url.indexOf(".sproute.io") == -1 &&
		...
		app.server.post("/admin/controller", function (req, res) {
			...
			for (var key in c) {
				if (!key) {
					return res.json(500, [{message: "Route must not be empty"}]);
				}

				if (!c[key]) {
					return res.json(500, [{message: "Page must not be empty"}]);
				}
		...
		app.server.delete("/admin/controller", function (req, res) {
			...
			if (!c.route) {
				return res.json(500, [{message: "Route must exist"}])
			}
